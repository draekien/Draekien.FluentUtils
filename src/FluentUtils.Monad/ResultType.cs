...
public abstract record ResultType<T> where T : notnull
{
    ...
    /// <summary>
    ///     Creates a new <see cref="ErrorResultType{T}" /> from an instance of
    ///     <see cref="Error" />
    /// </summary>
    ...
    /// <summary>
    ///     Creates a new <see cref="ErrorResultType{T}" /> from an exception,
    ///     automatically generating an error code and
    ...
    public static implicit operator ResultType<T>(Exception exception)
    {
        char[] upperCaseLetters =
            exception.GetType().Name.Where(char.IsUpper).ToArray();
